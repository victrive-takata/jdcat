			...
			case 'category':
				...
				switch (cat_name) {
					case 'item':
						...
						break;
					case 'description':
						title = '<h2>Usage Instructions</h2>';
						break;
					case 'caution':
						title = '<h2>Notes</h2>';
						break;
					case 'recommend':
						title = '<h2>Recommended Examples</h2>';
						break;
					case 'deprecate':
...
function tagMake(targetItem) {
	...
	switch (secL) {
		...
		case 'label':
			...
			var htmlContent = '<dl class="label"><dt>Usage:</dt><dd>' + label + "</dd></dl>";
			output.insertAdjacentHTML('beforeend', htmlContent);
			break;
		case 'repeat':
			var repeat = targetItem.textContent;
			var htmlContent = '<dl class="repeat"><dt>Occurrence:</dt><dd>' + repeat + "</dd></dl>";
			output.insertAdjacentHTML('beforeend', htmlContent);
			break;
		case 'above':
			var above = '<h4>Controlled Vocabularies</h4>';
